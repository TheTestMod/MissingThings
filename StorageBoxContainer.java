...
public class StorageBoxContainer extends Container {
	...
	private final int TE_INVENTORY_SLOT_ROW_COUNT = 6; //default 9
	private final int TE_INVENTORY_SLOT_COLUMN_COUNT = 9;
	private final int TE_INVENTORY_SLOT_COUNT = TE_INVENTORY_SLOT_ROW_COUNT * TE_INVENTORY_SLOT_COLUMN_COUNT;

	public StorageBoxContainer(InventoryPlayer invPlayer, StorageBoxTileEntity storageBoxTileEntity) {
		this.storageBoxTileEntity = storageBoxTileEntity;
    ...
    final int SLOT_Y_SPACING = 18;
		...
		if (TE_INVENTORY_SLOT_COUNT != storageBoxTileEntity.getSizeInventory()) {
			System.err.println("Mismatched slot count in StorageBoxContainer(" + TE_INVENTORY_SLOT_COUNT
												  + ") and TileInventory (" + storageBoxTileEntity.getSizeInventory()+")");
		}
		final int TILE_INVENTORY_XPOS = 8;
		final int TILE_INVENTORY_YPOS = 18;
		// Add the tile inventory container to the gui
		for (int y = 0; y < TE_INVENTORY_SLOT_ROW_COUNT; y++) {
			for (int x = 0; x < TE_INVENTORY_SLOT_COLUMN_COUNT; x++) {
				int slotNumber = x + y * 9;
				int xpos = TILE_INVENTORY_XPOS + x * SLOT_X_SPACING;
				int ypos = TILE_INVENTORY_YPOS + y * SLOT_Y_SPACING;
			addSlotToContainer(new Slot(storageBoxTileEntity, slotNumber, xpos, ypos));
			}
		}
	}
	...
	{
		return storageBoxTileEntity.isUsableByPlayer(player);
	}
	...
	public void onContainerClosed(EntityPlayer playerIn)
	{
		...
		this.storageBoxTileEntity.closeInventory(playerIn);
	}
}
